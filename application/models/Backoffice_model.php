...
class Backoffice_model extends CI_Model
{
	...
	public function modelCheckLogin($empcode, $password)
	{
		$sql = "select * from sys_staff_web where ss_emp_code ='{$empcode}' and ss_emp_password ='{$password}'";
	...
	public function modelSetPassword($email, $password)
	{
		// if($email =! ""){
			$sql = "select * from sys_staff_web where ss_email ='{$email}'";
			$res = $this->db->query($sql);
		...
		// exit();
			if ($row == null ){
				...
				$sql = "update sys_staff_web set ss_emp_password = '{$password}' where ss_email = '{$email}'";
				$res = $this->db->query($sql);
				return 'true';
			}
